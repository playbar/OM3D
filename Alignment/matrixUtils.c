...
void sparseMatrixVectorProduct( int mode, int m, int n, double x[], double y[], void* UsrWrk)
{
    // void* UsrWork contains the matrix as i1, j1, v1, i2, j2, v2, i3, j3, v3, etc.
    // which means A[i1,j1]=v1, A[i2,j2]=v2, etc.
    // Also, values of i will appear sorted in our case, due to the
    //      way we populate our matrix, this makes life easier for mode==1
    // values of j need not be sorted
    double* mywork=(double*)UsrWrk;
    if (mode<=2) {
        // compute y = y + Ax
        int k=0, count=0, l=0, indj;
        for (k=0; k<m; k++) {
            for (l=0; (int)(mywork[3*(l+count)])==k; l++) {
                if (mode==1) {
                    y[k]+=mywork[3*(l+count)+2]*x[ (int)( mywork[3*(l+count)+1] ) ];
                } else if (mode==2) {
                    x[ (int)( mywork[3*(l+count)+1] ) ]+=mywork[3*(l+count)+2]*y[k];
                }
            }
            count+=l;
        }
