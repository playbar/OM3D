...
- (id) init
{
	self=[super init];
	if (!self) return nil;
	[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(receivedNotification:) name:@"rotateNotification" object:nil];
	[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(receivedNotification:) name:@"scaleNotification" object:nil];
	...
	[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(receivedNotification:) name:@"PointsCameraNotification" object:nil];
...
- (void) receivedNotification:(NSNotification*)notification
{
	...
	} else if ([[notification name] isEqualToString:@"rotateNotification"])
    {
        [libraryObjectView setOperationMode:MODE_ROTATE];
        [libraryObjectView setNeedsDisplay:YES];
    } else if ([[notification name] isEqualToString:@"scaleNotification"])
    {
        [libraryObjectView setOperationMode:MODE_SCALE];
        [libraryObjectView setNeedsDisplay:YES];
    }
    else if ([[notification name] isEqualToString:@"PointsCameraNotification"])
    {
        if ([libraryObjectView getNumClickedPoints]>=4 && [imageView getNumClickedPoints]>=4) {
            [imageView enableCamera];
        }
    }
    else if ([[notification name] isEqualToString:@"pickPointsNotification"])
    {
        [libraryObjectView setOperationMode:MODE_PICKPOINTS];
        [libraryObjectView setNeedsDisplay:YES];
