...
BBox Sphere::ObjectBound() const {
    return BBox(PbrtPoint(-radius, -radius, zmin),
                PbrtPoint( radius,  radius, zmax));
}
...
PbrtPoint Sphere::Sample(float u1, float u2, Normal *ns) const {
    PbrtPoint p = PbrtPoint(0,0,0) + radius * UniformSampleSphere(u1, u2);
...
PbrtPoint Sphere::Sample(const PbrtPoint &p, float u1, float u2,
                     Normal *ns) const {
    // Compute coordinate system for sphere sampling
    PbrtPoint Pcenter = (*ObjectToWorld)(PbrtPoint(0,0,0));
...
float Sphere::Pdf(const PbrtPoint &p, const Vector &wi) const {
    PbrtPoint Pcenter = (*ObjectToWorld)(PbrtPoint(0,0,0));
