...
BBox Disk::ObjectBound() const {
    return BBox(PbrtPoint(-radius, -radius, height),
                PbrtPoint( radius,  radius, height));
}
...
PbrtPoint Disk::Sample(float u1, float u2, Normal *Ns) const {
    PbrtPoint p;
