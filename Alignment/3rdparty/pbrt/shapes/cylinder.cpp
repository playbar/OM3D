...
BBox Cylinder::ObjectBound() const {
    PbrtPoint p1 = PbrtPoint(-radius, -radius, zmin);
    PbrtPoint p2 = PbrtPoint( radius,  radius, zmax);
    return BBox(p1, p2);
}
...
PbrtPoint Cylinder::Sample(float u1, float u2, Normal *Ns) const {
    float z = Lerp(u1, zmin, zmax);
    float t = u2 * phiMax;
    PbrtPoint p = PbrtPoint(radius * cosf(t), radius * sinf(t), z);
