...
public:
    ...
    ~DiffuseAreaLight();
    Spectrum L(const PbrtPoint &p, const Normal &n, const Vector &w) const {
        return Dot(n, w) > 0.f ? Lemit : 0.f;
    }
    Spectrum Power(const Scene *) const;
    bool IsDeltaLight() const { return false; }
    float Pdf(const PbrtPoint &, const Vector &) const;
    Spectrum Sample_L(const PbrtPoint &P, float pEpsilon, const LightSample &ls, float time,
