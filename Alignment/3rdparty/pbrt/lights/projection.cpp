...
Spectrum ProjectionLight::Sample_L(const PbrtPoint &p, float pEpsilon,
...
Spectrum ProjectionLight::Projection(const Vector &w) const {
    ...
    PbrtPoint Pl = lightProjection(PbrtPoint(wl.x, wl.y, wl.z));
...
float ProjectionLight::Pdf(const PbrtPoint &, const Vector &) const {
    return 0.;
}
