...
public:
	...
	virtual bool IntersectQ(const Ray &ray, float *tHit,
                           float *rayEpsilon, float* bs) const;
    ...
    virtual PbrtPoint Sample(float u1, float u2, Normal *Ns) const {
        Severe("Unimplemented Shape::Sample() method called");
        return PbrtPoint();
    }
    virtual float Pdf(const PbrtPoint &Pshape) const {
        return 1.f / Area();
    }
    virtual PbrtPoint Sample(const PbrtPoint &P, float u1, float u2,
                         Normal *Ns) const {
        return Sample(u1, u2, Ns);
    }
    virtual float Pdf(const PbrtPoint &p, const Vector &wi) const;
