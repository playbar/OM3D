...
void ParamSet::AddPoint(const string &name, const PbrtPoint *data, int nItems) {
    ErasePoint(name);
    ADD_PARAM_TYPE(PbrtPoint, points);
}
...
const PbrtPoint *ParamSet::FindPoint(const string &name, int *nItems) const {
    LOOKUP_PTR(points);
}


PbrtPoint ParamSet::FindOnePoint(const string &name, const PbrtPoint &d) const {
    LOOKUP_ONE(points);
}
...
string ParamSet::ToString() const {
    ...
    for (i = 0; i < points.size(); ++i) {
        char *bufp = buf;
        *bufp = '\0';
        const Reference<ParamSetItem<PbrtPoint> > &item = points[i];
        typeString = "point ";
        // Print _ParamSetItem_ declaration, determine how many to print
        int nPrint = item->nItems;
        ret += string("\"");
