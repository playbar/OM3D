...
public:
    ...
    virtual Spectrum Sample_L(const PbrtPoint &p, float pEpsilon,
        const LightSample &ls, float time, Vector *wi, float *pdf,
        VisibilityTester *vis) const = 0;
    virtual Spectrum Power(const Scene *) const = 0;
    virtual bool IsDeltaLight() const = 0;
    virtual Spectrum Le(const RayDifferential &r) const;
    virtual float Pdf(const PbrtPoint &p, const Vector &wi) const = 0;
    ...
    virtual void SHProject(const PbrtPoint &p, float pEpsilon, int lmax,
...
struct VisibilityTester {
    ...
    void SetSegment(const PbrtPoint &p1, float eps1,
                    const PbrtPoint &p2, float eps2, float time) {
        float dist = Distance(p1, p2);
        r = Ray(p1, (p2-p1) / dist, eps1, dist * (1.f - eps2), time);
        Assert(!r.HasNaNs());
    }
    void SetRay(const PbrtPoint &p, float eps, const Vector &w, float time) {
...
public:
    ...
    virtual Spectrum L(const PbrtPoint &p, const Normal &n,
...
public:
    ...
    ~ShapeSet();
    PbrtPoint Sample(const PbrtPoint &p, const LightSample &ls, Normal *Ns) const;
    PbrtPoint Sample(const LightSample &ls, Normal *Ns) const;
    float Pdf(const PbrtPoint &p, const Vector &wi) const;
    float Pdf(const PbrtPoint &p) const;
private:
