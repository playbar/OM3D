...
struct Photon {
    Photon(const PbrtPoint &pp, const Spectrum &wt, const Vector &w)
        : p(pp), alpha(wt), wi(w) { }
    Photon() { }
    PbrtPoint p;
    Spectrum alpha;
...
struct RadiancePhoton {
    RadiancePhoton(const PbrtPoint &pp, const Normal &nn)
        : p(pp), n(nn), Lo(0.f) { }
    RadiancePhoton() { }
    PbrtPoint p;
    Normal n;
...
struct PhotonProcess {
    ...
    void operator()(const PbrtPoint &p, const Photon &photon, float dist2,
                    float &maxDistSquared);
    ClosePhoton *photons;
...
struct RadiancePhotonProcess {
    ...
    void operator()(const PbrtPoint &p, const RadiancePhoton &rp,
...
inline float kernel(const Photon *photon, const PbrtPoint &p, float maxDist2);
...
static Spectrum EPhoton(KdTree<Photon> *map, int count, int nLookup,
    ClosePhoton *lookupBuf, float maxDist2, const PbrtPoint &p, const Normal &n);
...
inline void PhotonProcess::operator()(const PbrtPoint &p,
...
inline float kernel(const Photon *photon, const PbrtPoint &p,
                    float maxDist2) {
...
Spectrum EPhoton(KdTree<Photon> *map, int count, int nLookup,
        ClosePhoton *lookupBuf, float maxDist2, const PbrtPoint &p,
        const Normal &n) {
    if (!map) return 0.f;
