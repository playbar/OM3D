...
struct SubsurfaceOctreeNode {
    ...
    void Insert(const BBox &nodeBound, IrradiancePoint *ip,
                MemoryArena &arena) {
        PbrtPoint pMid = .5f * nodeBound.pMin + .5f * nodeBound.pMax;
        if (isLeaf) {
    ...
    Spectrum Mo(const BBox &nodeBound, const PbrtPoint &p, const DiffusionReflectance &Rd,
                float maxError);
    ...
    if (filename != "") {
        ...
        if (ReadFloatFile(filename.c_str(), &fpts)) {
            ...
            for (u_int i = 0; i < fpts.size(); i += 8)
                pts.push_back(SurfacePoint(PbrtPoint(fpts[i], fpts[i+1], fpts[i+2]),
                                           Normal(fpts[i+3], fpts[i+4], fpts[i+5]),
                                           fpts[i+6], fpts[i+7]));
        }
    }
    if (pts.size() == 0) {
        PbrtPoint pCamera = camera->CameraToWorld(camera->shutterOpen,
                                              PbrtPoint(0, 0, 0));
...
Spectrum SubsurfaceOctreeNode::Mo(const BBox &nodeBound, const PbrtPoint &pt,
