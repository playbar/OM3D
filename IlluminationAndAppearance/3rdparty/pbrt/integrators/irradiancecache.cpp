...
struct IrradProcess {
    ...
    IrradProcess(const PbrtPoint &P, const Normal &N, float mw, float cmsad) {
        p = P;
        n = N;
        minWeight = mw;
...
struct IrradianceSample {
    ...
    IrradianceSample() { }
    IrradianceSample(const Spectrum &e, const PbrtPoint &P, const Normal &N,
...
Spectrum IrradianceCacheIntegrator::indirectLo(const PbrtPoint &p,
    ...
    if (!interpolateE(scene, p, ng, &E, &wi)) {
        ...
        PBRT_IRRADIANCE_CACHE_STARTED_COMPUTING_IRRADIANCE(const_cast<PbrtPoint *>(&p), const_cast<Normal *>(&ng));
        ...
        PBRT_IRRADIANCE_CACHE_FINISHED_COMPUTING_IRRADIANCE(const_cast<PbrtPoint *>(&p), const_cast<Normal *>(&ng));
        ...
        PBRT_IRRADIANCE_CACHE_ADDED_NEW_SAMPLE(const_cast<PbrtPoint *>(&p), const_cast<Normal *>(&ng), contribExtent, &E, &wAvg, pixelSpacing);
...
bool IrradianceCacheIntegrator::interpolateE(const Scene *scene,
        const PbrtPoint &p, const Normal &n, Spectrum *E,
        Vector *wi) const {
    if (!octree) return false;
    PBRT_IRRADIANCE_CACHE_STARTED_INTERPOLATION(const_cast<PbrtPoint *>(&p), const_cast<Normal *>(&n));
    IrradProcess proc(p, n, minWeight, cosMaxSampleAngleDifference);
    RWMutexLock lock(*mutex, READ);
    octree->Lookup(p, proc);
    PBRT_IRRADIANCE_CACHE_FINISHED_INTERPOLATION(const_cast<PbrtPoint *>(&p), const_cast<Normal *>(&n),
