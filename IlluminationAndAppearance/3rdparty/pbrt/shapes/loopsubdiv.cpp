...
struct SDVertex {
    ...
    SDVertex(PbrtPoint pt = PbrtPoint(0,0,0))
        : P(pt), startFace(NULL), child(NULL),
          regular(false), boundary(false) { }

    // SDVertex Methods
    int valence();
    void oneRing(PbrtPoint *P);
    PbrtPoint P;
    SDFace *startFace;
    SDVertex *child;
...
PbrtPoint LoopSubdiv::weightOneRing(SDVertex *vert, float beta) {
    // Put _vert_ one-ring in _Pring_
    int valence = vert->valence();
    PbrtPoint *Pring = ALLOCA(PbrtPoint, valence);
    vert->oneRing(Pring);
    PbrtPoint P = (1 - valence * beta) * vert->P;
...
PbrtPoint LoopSubdiv::weightBoundary(SDVertex *vert, float beta) {
    // Put _vert_ one-ring in _Pring_
    int valence = vert->valence();
    PbrtPoint *Pring = ALLOCA(PbrtPoint, valence);
    vert->oneRing(Pring);
    PbrtPoint P = (1-2*beta) * vert->P;
    P += beta * Pring[0];
...
LoopSubdiv *CreateLoopSubdivShape(const Transform *o2w, const Transform *w2o,
    ...
    const PbrtPoint *P = params.FindPoint("P", &nps);
    if (!vi || !P) return NULL;
