...
struct SurfacePoint {
    SurfacePoint() { }
    SurfacePoint(const PbrtPoint &pp, const Normal &nn, float a, float eps)
...
public:
    ...
    SurfacePointsRenderer(float md, const PbrtPoint &pc, float t,
                          const string &fn)
...
private:
    ...
    PbrtPoint pCamera;
    string filename;
    friend void FindPoissonPointDistribution(const PbrtPoint &pCamera, float time,
        float minDist, const Scene *scene, vector<SurfacePoint> *points);
    vector<SurfacePoint> points;
};


void FindPoissonPointDistribution(const PbrtPoint &pCamera, float time, float minDist,
