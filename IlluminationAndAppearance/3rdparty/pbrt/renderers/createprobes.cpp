...
public:
    CreateRadProbeTask(int i, int nProbes[3], float t, const BBox &b, int lmax, bool id,
           ...
           const vector<PbrtPoint> &sp, const Scene *sc, const Renderer *ren, Spectrum *c);
    void Run();
private:
    ...
    const Scene *scene;
    const vector<PbrtPoint> &surfacePoints;
    Spectrum *c_in;
};
...
void CreateRadianceProbes::Render(const Scene *scene) {
    ...
    vector<PbrtPoint> surfacePoints;
...
CreateRadianceProbes *CreateRadianceProbesRenderer(const Camera *camera,
    ...
    if (b) {
        ...
        else bounds = BBox(PbrtPoint(b[0], b[1], b[2]), PbrtPoint(b[3], b[4], b[5]));
    }
