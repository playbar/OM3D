...
PointLight::PointLight(const Transform &light2world,
    ...
    : Light(light2world) {
    lightPos = LightToWorld(PbrtPoint(0,0,0));
    Intensity = intensity;
}


Spectrum PointLight::Sample_L(const PbrtPoint &p, float pEpsilon,
...
PointLight *CreatePointLight(const Transform &light2world,
    ...
    PbrtPoint P = paramSet.FindOnePoint("from", PbrtPoint(0,0,0));
    Transform l2w = Translate(Vector(P.x, P.y, P.z)) * light2world;
    return new PointLight(l2w, I * sc);
}


float PointLight::Pdf(const PbrtPoint &, const Vector &) const {
    return 0.;
}
...
void PointLight::SHProject(const PbrtPoint &p, float pEpsilon, int lmax,
