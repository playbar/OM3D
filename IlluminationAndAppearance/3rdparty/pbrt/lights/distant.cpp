...
Spectrum DistantLight::Sample_L(const PbrtPoint &p, float pEpsilon,
...
DistantLight *CreateDistantLight(const Transform &light2world,
    ...
    PbrtPoint from = paramSet.FindOnePoint("from", PbrtPoint(0,0,0));
    PbrtPoint to = paramSet.FindOnePoint("to", PbrtPoint(0,0,1));
    Vector dir = from-to;
    return new DistantLight(light2world, L * sc, dir);
}


float DistantLight::Pdf(const PbrtPoint &, const Vector &) const {
    return 0.;
}
