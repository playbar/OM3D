...
private:
    void sphere(const PbrtPoint &P, float *s, float *t) const;
...
private:
    ...
    void cylinder(const PbrtPoint &p, float *s, float *t) const {
        Vector vec = Normalize(WorldToTexture(p) - PbrtPoint(0,0,0));
...
public:
    ...
    virtual PbrtPoint Map(const DifferentialGeometry &dg,
...
public:
    IdentityMapping3D(const Transform &x)
        : WorldToTexture(x) { }
    PbrtPoint Map(const DifferentialGeometry &dg, Vector *dpdx,
              Vector *dpdy) const;
private:
    Transform WorldToTexture;
};
...
float Noise(const PbrtPoint &P);
float FBm(const PbrtPoint &P, const Vector &dpdx, const Vector &dpdy,
    float omega, int octaves);
float Turbulence(const PbrtPoint &P, const Vector &dpdx, const Vector &dpdy,
    float omega, int octaves);
