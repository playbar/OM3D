...
class PbrtPoint {
public:
    // PbrtPoint Public Methods
    PbrtPoint() { x = y = z = 0.f; }
    PbrtPoint(float xx, float yy, float zz)
        : x(xx), y(yy), z(zz) {
        Assert(!HasNaNs());
    }
#ifndef NDEBUG
    PbrtPoint(const PbrtPoint &p) {
        Assert(!p.HasNaNs());
        x = p.x; y = p.y; z = p.z;
    }
    
    PbrtPoint &operator=(const PbrtPoint &p) {
        Assert(!p.HasNaNs());
        x = p.x; y = p.y; z = p.z;
        return *this;
    }
#endif // !NDEBUG
    PbrtPoint operator+(const Vector &v) const {
        Assert(!v.HasNaNs());
        return PbrtPoint(x + v.x, y + v.y, z + v.z);
    }
    
    PbrtPoint &operator+=(const Vector &v) {
        Assert(!v.HasNaNs());
        x += v.x; y += v.y; z += v.z;
        return *this;
    }
    Vector operator-(const PbrtPoint &p) const {
        Assert(!p.HasNaNs());
    ...
    PbrtPoint operator+(const PbrtPoint &p) const {
        Assert(!p.HasNaNs());
        return PbrtPoint(x + p.x, y + p.y, z + p.z);
    }
    PbrtPoint operator* (float f) const {
        return PbrtPoint(f*x, f*y, f*z);
    }
    PbrtPoint &operator*=(float f) {
        x *= f; y *= f; z *= f;
        return *this;
    }
    PbrtPoint operator/ (float f) const {
        float inv = 1.f/f;
        return PbrtPoint(inv*x, inv*y, inv*z);
    }
    PbrtPoint &operator/=(float f) {
        float inv = 1.f/f;
        x *= inv; y *= inv; z *= inv;
    ...
    bool operator==(const PbrtPoint &p) const {
...
public:
    ...
    RayDifferential(const PbrtPoint &org, const Vector &dir, float start,
        float end = INFINITY, float t = 0.f, int d = 0)
            : Ray(org, dir, start, end, t, d) {
        hasDifferentials = false;
    }
    RayDifferential(const PbrtPoint &org, const Vector &dir, const Ray &parent,
        float start, float end = INFINITY)
...
public:
    // BBox Public Methods
    BBox() {
        pMin = PbrtPoint( INFINITY,  INFINITY,  INFINITY);
        pMax = PbrtPoint(-INFINITY, -INFINITY, -INFINITY);
    }
    BBox(const PbrtPoint &p) : pMin(p), pMax(p) { }
    BBox(const PbrtPoint &p1, const PbrtPoint &p2) {
        pMin = PbrtPoint(min(p1.x, p2.x), min(p1.y, p2.y), min(p1.z, p2.z));
        pMax = PbrtPoint(max(p1.x, p2.x), max(p1.y, p2.y), max(p1.z, p2.z));
    }
    friend BBox Union(const BBox &b, const PbrtPoint &p);
    ...
    bool Inside(const PbrtPoint &pt) const {
    ...
    const PbrtPoint &operator[](int i) const;
    PbrtPoint &operator[](int i);
    PbrtPoint Lerp(float tx, float ty, float tz) const {
        return PbrtPoint(::Lerp(tx, pMin.x, pMax.x), ::Lerp(ty, pMin.y, pMax.y),
                     ::Lerp(tz, pMin.z, pMax.z));
    }
    Vector Offset(const PbrtPoint &p) const {
        return Vector((p.x - pMin.x) / (pMax.x - pMin.x),
                      (p.y - pMin.y) / (pMax.y - pMin.y),
                      (p.z - pMin.z) / (pMax.z - pMin.z));
    }
    void BoundingSphere(PbrtPoint *c, float *rad) const;
...
inline Vector::Vector(const PbrtPoint &p)
    : x(p.x), y(p.y), z(p.z) {
    Assert(!HasNaNs());
}
...
inline Vector Emult(const Vector &v1, const Vector &v2) {
	return Vector(v1.x*v2.x,v1.y*v2.y,v1.z*v2.z);
}

inline Vector Ediv(const Vector &v1, const Vector &v2) {
	return Vector(v1.x/v2.x,v1.y/v2.y,v1.z/v2.z);
}


inline float Distance(const PbrtPoint &p1, const PbrtPoint &p2) {
    return (p1 - p2).Length();
}


inline float DistanceSquared(const PbrtPoint &p1, const PbrtPoint &p2) {
    return (p1 - p2).LengthSquared();
}


inline PbrtPoint operator*(float f, const PbrtPoint &p) {
    Assert(!p.HasNaNs());
    return p*f;
}
...
inline const PbrtPoint &BBox::operator[](int i) const {
    Assert(i == 0 || i == 1);
    return (&pMin)[i];
}
...
inline PbrtPoint &BBox::operator[](int i) {
    Assert(i == 0 || i == 1);
    return (&pMin)[i];
}
...
inline float VonMises(const Vector &x, const Vector &mu, float kappa, float den=INFINITY) {
	if (den == INFINITY) {
		den=1.f/(4*M_PI*sinhf(kappa));
	}
	return kappa*den*exp(kappa*Dot(mu, x));
}
