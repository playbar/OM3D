...
public:
    ...
    virtual Spectrum sigma_a(const PbrtPoint &, const Vector &,
                             float time) const = 0;
    virtual Spectrum sigma_s(const PbrtPoint &, const Vector &,
                             float time) const = 0;
    virtual Spectrum Lve(const PbrtPoint &, const Vector &,
                         float time) const = 0;
    virtual float p(const PbrtPoint &, const Vector &,
                    const Vector &, float time) const = 0;
    virtual Spectrum sigma_t(const PbrtPoint &p, const Vector &wo, float time) const;
...
public:
    ...
    virtual float Density(const PbrtPoint &Pobj) const = 0;
    Spectrum sigma_a(const PbrtPoint &p, const Vector &, float) const {
        return Density(WorldToVolume(p)) * sig_a;
    }
    Spectrum sigma_s(const PbrtPoint &p, const Vector &, float) const {
        return Density(WorldToVolume(p)) * sig_s;
    }
    Spectrum sigma_t(const PbrtPoint &p, const Vector &, float) const {
        return Density(WorldToVolume(p)) * (sig_a + sig_s);
    }
    Spectrum Lve(const PbrtPoint &p, const Vector &, float) const {
        return Density(WorldToVolume(p)) * le;
    }
    float p(const PbrtPoint &p, const Vector &w, const Vector &wp, float) const {
...
public:
    ...
    Spectrum sigma_a(const PbrtPoint &, const Vector &, float) const;
    Spectrum sigma_s(const PbrtPoint &, const Vector &, float) const;
    Spectrum Lve(const PbrtPoint &, const Vector &, float) const;
    float p(const PbrtPoint &, const Vector &, const Vector &, float) const;
    Spectrum sigma_t(const PbrtPoint &, const Vector &, float) const;
