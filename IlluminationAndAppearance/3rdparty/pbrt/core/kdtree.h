...
public:
    ...
    template <typename LookupProc> void Lookup(const PbrtPoint &p,
...
private:
    ...
    template <typename LookupProc> void privateLookup(uint32_t nodeNum,
        const PbrtPoint &p, LookupProc &process, float &maxDistSquared) const;
...
void KdTree<NodeData>::Lookup(const PbrtPoint &p, LookupProc &proc,
...
template <typename NodeData> template <typename LookupProc>
void KdTree<NodeData>::privateLookup(uint32_t nodeNum, const PbrtPoint &p,
        LookupProc &process, float &maxDistSquared) const {
