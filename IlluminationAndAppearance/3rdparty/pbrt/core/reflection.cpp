...
struct IrregIsoProc {
    ...
    void operator()(const PbrtPoint &p, const IrregIsotropicBRDFSample &sample,
...
PbrtPoint BRDFRemap(const Vector &wo, const Vector &wi) {
    ...
    return PbrtPoint(sini * sino, dphi / M_PI, cosi * coso);
}


Spectrum IrregIsotropicBRDF::f(const Vector &wo,
                               const Vector &wi) const {
    PbrtPoint m = BRDFRemap(wo, wi);
