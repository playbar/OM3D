...
#undef NOT_ONE
    ...
    inline PbrtPoint operator()(const PbrtPoint &pt) const;
    inline void operator()(const PbrtPoint &pt, PbrtPoint *ptrans) const;
...
inline PbrtPoint Transform::operator()(const PbrtPoint &pt) const {
    ...
    Assert(wp != 0);
    if (wp == 1.) return PbrtPoint(xp, yp, zp);
    else          return PbrtPoint(xp, yp, zp)/wp;
}


inline void Transform::operator()(const PbrtPoint &pt,
                                  PbrtPoint *ptrans) const {
    float x = pt.x, y = pt.y, z = pt.z;
...
public:
    ...
    PbrtPoint operator()(float time, const PbrtPoint &p) const;
