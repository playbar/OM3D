...
bool Primitive::IntersectQ(const Ray &r, Intersection *in, float* barycentrics) const {
	Severe("Unimplemented Primitive::IntersectQ() method called!");
	return false;
}
...
bool GeometricPrimitive::IntersectQ(const Ray &r,
                                   Intersection *isect, float* barycentrics) const {
    float thit, rayEpsilon;
    if (!shape->IntersectQ(r, &thit, &rayEpsilon, barycentrics))
        return false;
    isect->primitive = this;
    isect->WorldToObject = *shape->WorldToObject;
    isect->ObjectToWorld = *shape->ObjectToWorld;
