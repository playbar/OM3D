...
void extractPixelSpaceAppearanceAndMasks( BVHAccel* bvhAccel, PbrtPoint* TextureVertices, int* TextureFaces, int* adjacencies,// geometry
                                         float* Itexartist, float* Btex, // image data structure
...
void iterativeIlluminationAppearanceEstimate(BVHAccel* bvhAccel, Vector* vertexNormals, int* Faces,
                                             ...
                                             Vector& albedo, Vector& groundAlbedo, Vector& ambientGroundAlbedo,
                                             param& parameters,
    ...
    for (i=0; i<npixels; i++) {
        ...
        if (didIntersect || Igroundmask[i]) {
			...
			if ( didIntersect ||
				...
				quadrature(intersectionPt, n, r, bvhAccel, diffuseLight, cTheta, sTheta, cPhi, sPhi, SphereRadius, ndivs, groundAxis, diffuseColors, groundPt, groundn, groundColorsNoObj);
			...
			if (didIntersect) {
                fprintf(falbedo, "%f %f %f %d %d %d %f %f %f %f %f %f %f %f %f %f %d\n",
                        ...
                        PixelSpaceAppearance[3*i], PixelSpaceAppearance[3*i+1], PixelSpaceAppearance[3*i+2],
                        .0f, .0f, .0f,
                        1.0f, fgroups==NULL ? 1 : fgroups[shapeId]);
                nobj++;
			} else {
				if (Ishadowmask[i] || ( (int)xpixim % nstep==0 && (int)ypixim % nstep==0)) {
					fprintf(falbedo, "%f %f %f %d %d %d %f %f %f %f %f %f %f %f %f %f %d\n",
                            ...
                            .0f, .0f, .0f,
                            Ishadowmask[i] ? 1.0 : parameters.shadowweight, -1
							);
...
void completeAppearance(BVHAccel* bvhAccel, BVHAccel* texAccel, Vector* vertexNormals, Vector* vertexNormalsOrig,
	...
	for (i=0; i<npixels; i++) {
		...
		if (*(cvGet2D(middilatedImage, xpixim, ypixim)).val<.5f && !Igroundmask[i]) {
			...
			if (*(cvGet2D(middilatedImage, xpixim, ypixim)).val>.5f) {
				//foreground
				color.x=diffuseLightOutput[0][3*i]*PixelSpaceAppearance[3*i]+ambientLight.x*PixelSpaceAppearance[3*i];
				color.y=diffuseLightOutput[0][3*i+1]*PixelSpaceAppearance[3*i+1]+ambientLight.y*PixelSpaceAppearance[3*i+1];
				color.z=diffuseLightOutput[0][3*i+2]*PixelSpaceAppearance[3*i+2]+ambientLight.z*PixelSpaceAppearance[3*i+2];
				...
				if (Igroundmask[i]) {
					...
					Igroundtex[3*i]-=groundAlbedo.x*groundNoObjLightOutput[0][3*i]+ambientLight.x*ambientGroundAlbedo.x;
                    Igroundtex[3*i+1]-=groundAlbedo.y*groundNoObjLightOutput[0][3*i+1]+ambientLight.y*ambientGroundAlbedo.y;
                    Igroundtex[3*i+2]-=groundAlbedo.z*groundNoObjLightOutput[0][3*i+2]+ambientLight.z*ambientGroundAlbedo.z;
    ...
    for (i=0; i<ntexpixel; i++) {
        ...
        if (didIntersect) {
            ...
            if (didIntersect && shapeId != (isect->shapeId-shapeIdOffset)) {
                ...
                color=quadrature(vertext, n, r, bvhAccel, diffuseLight[0], cTheta, sTheta, cPhi, sPhi, SphereRadius, ndivs, groundAxis);
    ...
    float* layers=new float[3*ntexpixel*(int)(pow(2,nrounds))];
    for (int i=0; i<3*ntexpixel*(int)(pow(2,nrounds)); i++) {
        layers[i]=.0f;
    }
    for (int i=0; i<3*ntexpixel; i++) {
        layers[i]=Itexartist[i]+Itexdiff[i];
    }
    ...
    writeFile(direc, layers, 3*ntexpixel*(int)(pow(2,nrounds))*sizeof(float), "layers.dat");
    delete[] layers;
						...
						if (ndots>0) {
							...
							if (aboveground) {
								...
								if (!didLightIntersect) {
									...
									BigDiffuseTR[npixelsused*ndivs+j]=a1*ndots*sqrtf(shadowweighti);
									BigDiffuseTG[npixelsused*ndivs+j]=a2*ndots*sqrtf(shadowweighti);
									BigDiffuseTB[npixelsused*ndivs+j]=a3*ndots*sqrtf(shadowweighti);
                                }
							}
						}
