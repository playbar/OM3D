...
void SHProjectCube(Func func, const PbrtPoint &p, int res, int lmax,
                   Spectrum *coeffs) {
...
void SHProjectIncidentDirectRadiance(const PbrtPoint &p, float pEpsilon, float time,
    MemoryArena &arena, const Scene *scene, bool computeLightVisibility,
    int lmax, RNG &rng, Spectrum *c_d);
void SHProjectIncidentIndirectRadiance(const PbrtPoint &p, float pEpsilon,
...
void SHComputeDiffuseTransfer(const PbrtPoint &p, const Normal &n, float rayEpsilon,
    const Scene *scene, RNG &rng, int nSamples, int lmax, Spectrum *c_transfer);
void SHComputeTransferMatrix(const PbrtPoint &p, float rayEpsilon,
