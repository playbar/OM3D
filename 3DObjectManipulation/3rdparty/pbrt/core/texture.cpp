...
void SphericalMapping2D::sphere(const PbrtPoint &p, float *s, float *t) const {
...
void PlanarMapping2D::Map(const DifferentialGeometry &dg,
        float *s, float *t, float *dsdx, float *dtdx,
        float *dsdy, float *dtdy) const {
    Vector vec = dg.p - PbrtPoint(0,0,0);
    *s = ds + Dot(vec, vs);
    *t = dt + Dot(vec, vt);
    *dsdx = Dot(dg.dpdx, vs);
    *dtdx = Dot(dg.dpdx, vt);
...
PbrtPoint IdentityMapping3D::Map(const DifferentialGeometry &dg,
...
float Noise(const PbrtPoint &P) { return Noise(P.x, P.y, P.z); }
inline float Grad(int x, int y, int z, float dx, float dy, float dz) {
