...
Spectrum DiffuseAreaLight::Sample_L(const PbrtPoint &p, float pEpsilon,
        const LightSample &ls, float time, Vector *wi, float *pdf,
        VisibilityTester *visibility) const {
    PBRT_AREA_LIGHT_STARTED_SAMPLE();
    Normal ns;
    PbrtPoint ps = shapeSet->Sample(p, ls, &ns);
    *wi = Normalize(ps - p);
...
float DiffuseAreaLight::Pdf(const PbrtPoint &p, const Vector &wi) const {
...
Spectrum DiffuseAreaLight::Sample_L(const Scene *scene,
    ...
    PBRT_AREA_LIGHT_STARTED_SAMPLE();
    PbrtPoint org = shapeSet->Sample(ls, Ns);
