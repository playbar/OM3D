...
TriangleMesh::TriangleMesh(const Transform *o2w, const Transform *w2o,
        bool ro, int nt, int nv, const int *vi, const PbrtPoint *P,
...
BBox Triangle::ObjectBound() const {
    ...
    const PbrtPoint &p1 = mesh->p[v[0]];
    const PbrtPoint &p2 = mesh->p[v[1]];
    const PbrtPoint &p3 = mesh->p[v[2]];
...
bool Triangle::IntersectQ(const Ray &ray, float *tHit, float *rayEpsilon,
                         float* bs) const {
    PBRT_RAY_TRIANGLE_INTERSECTION_TEST(const_cast<Ray *>(&ray), const_cast<Triangle *>(this));
    // Compute $\VEC{s}_1$
...
TriangleMesh *CreateTriangleMeshShape(const Transform *o2w, const Transform *w2o,
    ...
    const PbrtPoint *P = params.FindPoint("P", &npi);
...
PbrtPoint Triangle::Sample(float u1, float u2, Normal *Ns) const {
    float b1, b2;
    ...
    const PbrtPoint &p2 = mesh->p[v[1]];
    const PbrtPoint &p3 = mesh->p[v[2]];
    PbrtPoint p = b1 * p1 + b2 * p2 + (1.f - b1 - b2) * p3;
