...
NURBSEvaluateSurface(int uOrder, const float *uKnot, int ucp, float u,
    ...
    return PbrtPoint(P.x/P.w, P.y/P.w, P.z/P.w);;
}
...
BBox NURBS::ObjectBound() const {
    if (!isHomogeneous) {
        ...
        for (int i = 0; i < nu*nv; ++i, pp += 3)
            bound = Union(bound, PbrtPoint(pp[0], pp[1], pp[2]));
        return bound;
    } else {
        ...
        for (int i = 0; i < nu*nv; ++i, pp += 4)
            bound = Union(bound, PbrtPoint(pp[0] / pp[3], pp[1] / pp[3], pp[2] / pp[3]));
        return bound;
    }
}
...
BBox NURBS::WorldBound() const {
    if (!isHomogeneous) {
        ...
        BBox bound;
        for (int i = 0; i < nu*nv; ++i, pp += 3) {
            PbrtPoint pt = (*ObjectToWorld)(PbrtPoint(pp[0], pp[1], pp[2]));
        ...
        BBox bound;
        for (int i = 0; i < nu*nv; ++i, pp += 4) {
            PbrtPoint pt = (*ObjectToWorld)(PbrtPoint(pp[0]/pp[3],
                pp[1]/pp[3], pp[2]/pp[3]));
            bound = Union(bound, pt);
        }
        return bound;
...
void NURBS::Refine(vector<Reference<Shape> > &refined) const {
    ...
    PbrtPoint *evalPs = new PbrtPoint[diceu*dicev];
    ...
    memset(evalPs, 0, diceu*dicev*sizeof(PbrtPoint));
    memset(evalNs, 0, diceu*dicev*sizeof(PbrtPoint));
    ...
    for (int v = 0; v < dicev; ++v) {
        for (int u = 0; u < diceu; ++u) {
            ...
            Vector dPdu, dPdv;
            PbrtPoint pt = NURBSEvaluateSurface(uorder, uknot, nu, ueval[u],
