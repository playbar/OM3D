...
PerspectiveCamera:: PerspectiveCamera(const AnimatedTransform &cam2world,
    ...
    dxCamera = RasterToCamera(PbrtPoint(1,0,0)) - RasterToCamera(PbrtPoint(0,0,0));
    dyCamera = RasterToCamera(PbrtPoint(0,1,0)) - RasterToCamera(PbrtPoint(0,0,0));
}
...
float PerspectiveCamera::GenerateRay(const CameraSample &sample,
    ...
    *ray = Ray(PbrtPoint(0,0,0), Normalize(Vector(Pcamera)), 0.f, INFINITY);
...
float PerspectiveCamera::GenerateRayDifferential(const CameraSample &sample,
    ...
    *ray = RayDifferential(PbrtPoint(0,0,0), dir, 0.f, INFINITY);
    // Modify ray for depth of field
    if (lensRadius > 0.) {
        // Sample point on lens
        float lensU, lensV;
